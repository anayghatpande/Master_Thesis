...
def json_loader():
    ...
    for path in dataset_path:
        ...
        print(json_path)
        file = open(json_path)
        #shutil.rmtree(path + "/pcd_transformed", ignore_errors=True)
        #print("Removing old PCD_transformed")
        #os.mkdir(path + "/pcd_transformed")
        dict = json.load(file)
        ...
        for scene_id in range(len(dict)):
            ...
            for no_of_objs in range(len(dict[str(scene_id)])):
                ...
                object_data = transform(matR, matT, objects, scene_id, path)
...
def transform(matr, matt, objts, scn_id, path):
    #print(path + "/pcd/" + str(scn_id) + ".pcd", objts)
    pcd_path = path+"/pcd/" + str(scn_id) + ".pcd"
   ...
   # o3d.io.write_point_cloud(str(path + "/pcd_transformed") + "/" + str(j) + ".pcd", pcd)
    #print()
